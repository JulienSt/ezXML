...
object CTConverter {
    ...
    def xmlImpl[A](c: blackbox.Context)(a: c.Expr[A])(implicit ATag : c.WeakTypeTag[A]) : c.Expr[Elem] = {
        import c.universe._
        c.Expr[Elem](q"indv.jstengel.ezxml.extension.ct.CTConverter.xmlMacro[${ATag.tpe}]($a)")
    }
    ...
    def xmlAnnotation[A]: A => Elem = macro convertAnnotationImpl[A]
    ...
    def convertImpl[A] (c: blackbox.Context)
        ...
        convertClassToXML(c)(None, None, false)
    /**
     *
     * @param c
     * @param ATag
    ...
    def convertAnnotationImpl[A] (c: blackbox.Context)
                                 (implicit ATag: c.WeakTypeTag[A]): c.Expr[A => Elem] =
        convertClassToXML(c)(None, None, isCalledFromAnnotation = true)
    ...
    def convertWithMappingImpl[A] (c : blackbox.Context)
                                  ...
                                  (implicit ATag: c.WeakTypeTag[A]): c.Expr[A => Elem] =
        convertClassToXML(c)(Some(mappings), None, false)
    ...
    def convertWithFieldImpl[A] (c : blackbox.Context)
                                ...
                                (implicit ATag: c.WeakTypeTag[A]): c.Expr[A => Elem] =
        convertClassToXML(c)(None, Some(fieldName), false)
    ...
    def convertWithAllImpl[A] (c : blackbox.Context)
                              (mappings  : c.Expr[FieldMappings],
                               fieldName : c.Expr[String])
                              (implicit ATag : c.WeakTypeTag[A]) : c.Expr[A => Elem] =
        convertClassToXML(c)(Some(mappings), Some(fieldName), false)
    ...
    def convertClassToXML[A] (c : blackbox.Context)
        ...
        val fullTypeName = createStringRepresentation(aType)(typeParams) // todo is missing typeParameter in string
        val isCallingEnclosingClass = isMacroCallingEnclosingClass(c)(aType)
        
        println("------------------------")
        println(fullTypeName)
        println(isSimple(c)(aType))
        
        if (! isCallingEnclosingClass && aType <:< typeOf[XmlClassTrait]) {
            println(1)
...
//                            """
                        ...
                        if (isSimple(c)(fieldType))
                            ...
                            q"""val scala.xml.Elem(prefix, label, attribs, scope, child @ _*) = $quote
                                ...
                                .CTConverter.xmlMacroAsField[${TypeName(fieldTypeAsString)}](${fName.toString})($fieldCall): _*)"""
                        else
                            q"""val scala.xml.Elem(prefix, label, attribs, scope, child @ _*) = $quote
                                scala.xml.Elem(prefix, label, attribs, scope, false, child ++
                                indv.jstengel.ezxml.extension.ct
                                .CTConverter.xmlMacroAsField[$fieldType](${fName.toString})($fieldCall): _*)"""
                    }
                
                val elemAsFunctionString = q"(objectToBeEncoded: $aType) => $elemAsString"
                
//                println("------------------------------------------")
//                println(elemAsFunctionString)
                elemAsFunctionString
    ...
    def createRuntimeConversion[A] (c : blackbox.Context)
                                   (tpe : c.Type,
                                    ...
                                    fieldName : Option[c.Expr[String]])
                                   (implicit ATag: c.WeakTypeTag[A]): c.Expr[A => Elem] = { import c.universe._
        c.Expr[A => Elem](q""" (objectToBeEncoded: $tpe) => indv.jstengel.ezxml.extension.rt.RTConverter.convertToXML(
            objectToBeEncoded,
            ${mappings.getOrElse(c.Expr[FieldMappings](q"Seq()"))},
